# writ Extraction — Review & Analysis (v0.1)

_Source reviewed: `redrix-dev/haven` @ `87f657f`. Scaffold reviewed:
`redrix-dev/writ` @ `124bbb5`._
...
- a **convention** (per-domain `Xxxwrit` classes),
- a **thin base class** (`apps/mobile/src/data/writ.ts`, ~156 lines,
  Zustand-coupled),
...
## 1. Public surface of "writ" as it exists in Haven
...
| Entity base class          | `writ<T,R>` (`apps/mobile/src/data/writ.ts`)                        | none — each writ holds a `createStore` directly                                  |
...
### The recurring shape of one writ (this is the "unit")

Every domain writ, on both platforms, is the same five things in one file:
...
`packages/solid-client/src/data/channels/channelSolidwrit.ts` (Solid template)
and `apps/mobile/src/data/channels/Channelwrit.ts` (React template).

### The reusable primitives (what's actually generic)

Stripped of domain, these are the pieces worth extracting:

- **Entity map + CRUD** — `Record<id, writEntry<T>>` where
  `writEntry = { data, partial, cachedAt }`; ops `getOrCreate` / `getOrPartial`
  / `update` / `delete` / `has` / `getSnapshot` (`writ.ts`,
  `core/cache/entityTypes.ts`).
- **One-writer boundary** —
  `ReadableStore<S> = Pick<StoreApi<S>, "getState" | "getInitialState" | "subscribe">`
  (`packages/shared/src/writ/storeTypes.ts`). This is the whole authority
  mechanism, and it's ~4 lines.
- **Persistence port** — `writPersistence { getString/set/remove }`
  (`core/persistence/writPersistence.ts`) + in-memory/localStorage adapters.
...
1. **Zustand & Solid as the substrate.** `writ.ts` imports `zustand`; Solid
   writes import `solid-js/store`. Core must ship its **own** tiny vanilla store
   (getState / subscribe / private set) so "zero dependencies" is true and the
   React adapter is a `useSyncExternalStore` one-liner.
...
4. **`writEntry.partial` / `cachedAt`** — the partial-hydration + cache-age
   model is a Haven data-fetching concern. writ core should treat entities as
...
6. **Storage key convention** `haven:writ:<type>:<instance>` and
   `writ_STORAGE_KEYS` — Haven namespacing. Core takes a caller-supplied key.
...
- _"A writ fetches from a backend."_ No — a writ **owns state**. Fetching is one
...
- _"One writer == one domain store."_ Haven enforces one-writer at the **store**
  granularity (the Channelwrit owns _all_ channels; any holder can update any
...
// ── persistence port (Haven's writPersistence, unchanged) ────────
...
1. **Persistence port + memory adapter.** Smallest, zero-risk, unblocks tests.
   (Direct lift of `writPersistence` + `createMemoryPersistence`.)
...
3. **Entity store** (`createEntityStore`: spawn/update/destroy/clear + entity
   map + optional persistence). Port CRUD semantics from `writ.ts`, drop
...
6. **Before/after demo** — chat-channel-with-presence slice: god-hook version
   vs. writ version, side by side (presence = entities being born/dying, so
...
## 5. Scaffold review (`redrix-dev/writ`) vs. modern TS-library standards
...
- `"name": "writ"` **cannot be published unscoped** — it is used by an unrelated
  literate-programming package. The selected scoped package name is
  `@redrixx/writ`.
...
4. **Package name** — resolved as `@redrixx/writ` (`writ` is taken unscoped).
